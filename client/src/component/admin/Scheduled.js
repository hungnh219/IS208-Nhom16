    ...
    <div className="flex bg-[#e6e6ee] w-[100%] min-h-[100vh] ">
      ...
      <div className=" basis-[100%] bg-[white] ml-[300px] mt-[24px] mr-[24px] p-[16px] rounded-[8px] shadow-xl">
                ...
                <tr key={index}>
                  {row.map((cell, idx) => (
                    idx>0?<td onClick={()=>{
                      ...
                      setDate(day[idx-1])
                      setClasses(row[idx])
                    }}key={idx}>{cell}</td>:<td key={idx}>{cell}</td>
                  ))}
                </tr>
              ))}
            </tbody>
        ...
        <div className="flex justify-start gap-[16px] mt-[32px] px-[40px]">
          ...
          <div className="font-bold">Lớp: 
            <select  className="border-[1px] border-black"value={classes}
            onChange={(e)=>{setChange(true);setClasses(e.target.value)}}>
        ...
        <button className=" px-[16px] py-[4px] ml-[16px] hover:opacity-[0.8] bg-[#247afb] text-[white] shadow-xl mr-[8px]"
            onClick={()=>{
              if(change){
                ...
                else toast.error("Vui lòng thay đổi trước khi cập nhật!",{
                  autoClose: 1000,
                });
                }}>Cập nhật</button>
