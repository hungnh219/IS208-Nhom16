...
const Notebook = () => {
  ...
  let indexComment;
  let indexContent;
  notebook[days].forEach((item, idx) => {
    if (item.lesson == lesson) indexComment = idx;
  });
  notebook[days].forEach((item, idx) => {
    if (item.lesson == lesson) indexContent = idx;
  });
    ...
    <div className="flex bg-[#e6e6ee] w-[100%] min-h-[100vh]">
      ...
      <div className="basis-[100%] bg-[white] ml-[300px] mt-[24px] mr-[24px] p-[16px] rounded-[8px] shadow-xl">
        <div className="flex justify-center">
          ...
          <label className="font-bold mr-[8px] ml-[16px]">
            Năm học: 2023-2024
          </label>
        </div>
              ...
              {Object.keys(groupDays).map((key, index) => {
                return groupDays[key].map((item, id) => {
                  return (
                    <tr key={index}>
                      {id === 0 ? (
                        <td rowspan={groupDays[key].length}>{item.day}</td>
                      ) : (
                        ""
                      )}
                      <td>{item.lesson}</td>
      ...
      >
        ...
        <button
          onClick={() => setModalShowIsOpen(false)}
      ...
      >
        <div className="p-[16px] ">
          ...
          <div className="py-[4px]">
            <label className="font-bold">Nội dung bài học</label>
            <textarea
              className="w-[100%] h-[100px] px-[8px] py-[8px] mt-[8px] rounded bg-[#ececec]"
              onChange={(e) => {
                setNotebook((prev) => {
                  let index = 0;
                  const note = { ...prev };
                  note[days].forEach((item, idx) => {
                    if (item.lesson == lesson) index = idx;
                  });
      ...
      >
        ...
        <button
          onClick={() => setModalAttendIsOpen(false)}
          className="absolute top-[15px] right-[20px] hover:text-[blue]"
        >
          Đóng
        </button>
